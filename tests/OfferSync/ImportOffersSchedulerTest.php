<?php
/**
 * Testy jednostkowe czystej logiki OfferSync\ImportOffersScheduler (P-15.3a):
...
use Qutlet\Allegro\OfferSync\ImportOffersScheduler;
use PHPUnit\Framework\TestCase;

/**
 * Charakteryzuje konfigurowalność środowisk harmonogramu delta-checku importu
 * (D-15.5). Testy BEZ WordPressa — obie metody są czystymi funkcjami; efekty
 * uboczne (odczyt stałej, `WP_CLI::warning`/`error`) żyją w cienkim
 * `configured_environments()`, poza zasięgiem tego harnessu (bootstrap = sam
 * autoloader Composera). Wzorzec 1:1 z `StockSyncSchedulerTest`.
 *
 ...
 */
final class ImportOffersSchedulerTest extends TestCase {
	...
	public function test_parse_single_valid_environment(): void {
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( Environment::PRODUCTION )
		);
	}
	...
	public function test_parse_both_environments_reordered_to_canonical(): void {
		...
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( 'sandbox,production' )
		);
	}
	...
	public function test_parse_trims_whitespace_and_ignores_trailing_comma(): void {
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( '  production , ' )
		);
	}
	...
	public function test_parse_normalizes_case(): void {
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( 'PRODUCTION,Sandbox' )
		);
	}
	...
	public function test_parse_deduplicates_valid_and_invalid(): void {
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( 'production,production,foo,foo' )
		);
	}
	...
	public function test_parse_mixed_valid_and_invalid_tokens(): void {
		...
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( 'production,prdukcja' )
		);
	}
	...
	public function test_parse_all_invalid(): void {
		$this->assertSame(
			...
			ImportOffersScheduler::parse_environment_list( 'foo,bar' )
		);
	}
	...
	public function test_parse_empty_and_whitespace_only_yield_nothing(): void {
		...
		$this->assertSame( $empty, ImportOffersScheduler::parse_environment_list( '' ) );
		$this->assertSame( $empty, ImportOffersScheduler::parse_environment_list( '   ' ) );
		$this->assertSame( $empty, ImportOffersScheduler::parse_environment_list( ' , , ' ) );
	}
	...
	public function test_plan_undefined_constant_falls_back_to_both(): void {
		$plan = ImportOffersScheduler::plan_environments( false, null );
	...
	public function test_plan_undefined_ignores_any_raw_value(): void {
		...
		$plan = ImportOffersScheduler::plan_environments( false, 'foo' );
	...
	public function test_plan_valid_subset_runs_without_warning(): void {
		$plan = ImportOffersScheduler::plan_environments( true, 'production' );
	...
	public function test_plan_mixed_runs_valid_subset_and_warns(): void {
		$plan = ImportOffersScheduler::plan_environments( true, 'production,prdukcja' );
	...
	public function test_plan_all_invalid_is_hard_error(): void {
		$plan = ImportOffersScheduler::plan_environments( true, 'foo,bar' );
	...
	public function test_plan_empty_string_is_hard_error(): void {
		$plan = ImportOffersScheduler::plan_environments( true, '' );
	...
	public function test_plan_non_string_value_is_hard_error(): void {
		$plan = ImportOffersScheduler::plan_environments( true, array( 'production' ) );
